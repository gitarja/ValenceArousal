...
from Baseline.SingleModel import FeaturesSingleModel
...
for fold in range(1, 6):
    ...
    prev_val_loss = 1000
    wait_i = 0
    result_path = TRAINING_RESULTS_PATH + "Baseline\\teacher\\" + current_time + "\\fold_" + str(fold) + "\\"
    ...
    with strategy.scope():
        classification = FeaturesSingleModel(num_output_ar, num_output_val)
        input_tensor = tf.keras.layers.Input(shape=(FEATURES_N,))
        classification.createModel(input_tensor)
        ...
        # Manager
        checkpoint = tf.train.Checkpoint(step=tf.Variable(1), optimizer=optimizer, teacher_model=classification.model)
    ...
    with strategy.scope():
        def train_step(inputs, GLOBAL_BATCH_SIZE=0):
            ...
            with tf.GradientTape() as tape_ar:
                final_loss, loss_ar, loss_val, prediction_ar, prediction_val = classification.computeLoss(X, y_ar, y_val, GLOBAL_BATCH_SIZE)

            # update gradient
            grads = tape_ar.gradient(final_loss, classification.model.trainable_weights)
            optimizer.apply_gradients(zip(grads, classification.model.trainable_weights))
        ...
        def test_step(inputs, GLOBAL_BATCH_SIZE=0):
            ...
            final_loss, loss_ar, loss_val, prediction_ar, prediction_val = classification.computeLoss(X, y_ar, y_val,
                                                                                                      GLOBAL_BATCH_SIZE)
    ...
    with strategy.scope():
        ...
        for epoch in range(EPOCHS):
            ...
            template = "epoch {}/{} | Train_loss: {}, Train_acc_ar: {:.3%} | Val_loss: {}, Val_acc_ar: {:.3%}"
            print(template.format(epoch + 1, EPOCHS, train_loss.result().numpy(), train_ar_acc.result().numpy(), vald_loss.result().numpy(), vald_ar_acc.result().numpy()))
