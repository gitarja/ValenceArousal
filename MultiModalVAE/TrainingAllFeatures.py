...
from MultiModalVAE.MultiModalVAEModel import MultiVAEModel
...
for fold in range(1, 2):
    ...
    prev_val_loss = 1000
    wait_i = 0
    result_path = TRAINING_RESULTS_PATH + "SingleVAE\\fold_" + str(fold) + "\\"
    ...
    with strategy.scope():
        model = MultiVAEModel(num_output=num_output, features_length=FEATURES_N)
    ...
    with strategy.scope():
        def train_step(inputs, shake_params, GLOBAL_BATCH_SIZE=0):
            ...
            with tf.GradientTape() as tape_ar:
                z_em, z_r_ar, z_r_val, rec_X, _, z_mean, z_logvar = model(X, training=True)
                ...
                mse_loss, regress_loss = model.regressionLoss(z_r_ar, z_r_val, y_r_ar, y_r_val,
                                                              shake_params=shake_params,
                                                              global_batch_size=GLOBAL_BATCH_SIZE)
                # Autoencoder loss
                vae_loss = model.vaeLoss(X, rec_X, z_mean, z_logvar, global_batch_size=GLOBAL_BATCH_SIZE)

                final_loss = regress_loss + classific_loss + vae_loss
        ...
        def test_step(inputs, GLOBAL_BATCH_SIZE=0):
            ...
            y_r_ar = tf.expand_dims(inputs[2], -1)
            y_r_val = tf.expand_dims(inputs[3], -1)
            z_em, z_r_ar, z_r_val, _, _, _, _ = model(X, training=False)
            classific_loss = model.classificationLoss(z_em, y_emotion, global_batch_size=GLOBAL_BATCH_SIZE)
