...
from MultiModalVAE.VAELoss import KLLoss


class UnitModel(tf.keras.layers.Layer):
    def __init__(self, en_units=(32, 32, 16, 16), num_output=4, num_latent=16, features_length=2480, decoder=True, embedding_n=32,
                 **kwargs):
        ...
        self.latent_mean = tf.keras.layers.Dense(units=num_latent, name="latent_mean", activation=None)
        self.latent_logvar = tf.keras.layers.Dense(units=num_latent, name="latent_logvar", activation=None)
        self.latent = tf.keras.layers.Lambda(self.sampling, output_shape=(num_latent,))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.decoder:
            return z_em, z_ar, z_val, x, z, z_mean, z_logvar
        return z_em, z_ar, z_val, None, z, z_mean, z_logvar
...
class MultiVAEModel(tf.keras.Model):

    def __init__(self, num_output=4, features_length=2480, decoder=True):
        super(MultiVAEModel, self).__init__(self)
        self.decoder = decoder
        # DNN unit

        self.unit_small = UnitModel(en_units=(32, 32, 16, 16), num_output=num_output, embedding_n=32,
                                    features_length=features_length, name="unit_small", decoder=decoder)
        self.unit_medium = UnitModel(en_units=(64, 32, 16, 16), num_output=num_output, embedding_n=32,
                                     features_length=features_length, name="unit_medium", decoder=decoder)
        self.unit_large = UnitModel(en_units=(64, 64, 32, 16), num_output=num_output, embedding_n=32,
        ...
        self.bce_loss = tf.losses.BinaryCrossentropy(reduction=tf.keras.losses.Reduction.NONE)
        self.kl_loss = KLLoss(reduction=tf.keras.losses.Reduction.NONE)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        z_em, z_ar, z_val, x, z, z_mean, z_logvar = self.unit_small(inputs, training=training)
    ...
    def vaeLoss(self, x, x_re, z_mean, z_logvar, global_batch_size):
        # Kullback-Leibler loss
        kl_loss = tf.nn.compute_average_loss(self.kl_loss(z_mean, z_logvar), global_batch_size=global_batch_size) / global_batch_size

        # Reconstruction loss
        reconstruction_loss = tf.nn.compute_average_loss(self.bce_loss(x, x_re), global_batch_size=global_batch_size)
