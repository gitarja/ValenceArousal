...
from Conf.Settings import FEATURES_N, DATASET_PATH, CHECK_POINT_PATH, TENSORBOARD_PATH, ECG_RAW_N, TRAINING_RESULTS_PATH, N_CLASS, ECG_N, EDA_N
...
for fold in range(1, 6):
    ...
    checkpoint_prefix = result_path + "model_student_ECG_EDA_KD"

    # tensorboard
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    train_log_dir = result_path + "tensorboard_student_ECG_EDA_KD\\" + current_time + '/train'
    test_log_dir = result_path + "tensorboard_student_ECG_EDA_KD\\" + current_time + '/test'
    ...
    data_fetch = DataFetch(train_file=training_data, test_file=testing_data, validation_file=validation_data,
                           ECG_N=ECG_RAW_N, KD=True, teacher=False, ECG=True, EDA=True, high_only=False)
    ...
    train_generator = tf.data.Dataset.from_generator(
        ...
        output_shapes=(
        tf.TensorShape([FEATURES_N]), (tf.TensorShape([N_CLASS])), (), (), tf.TensorShape([ECG_N + EDA_N]), ()))
    ...
    with strategy.scope():
        ...
        def test_step(inputs, GLOBAL_BATCH_SIZE):
            ...
            update_test_metrics(classific_loss + mse_loss, z=[tf.nn.sigmoid(z_em), z_r_ar, z_r_val],
                                y=[y_emotion, y_r_ar, y_r_val])
    ...
    with strategy.scope():
        ...
        sys.stdout = open(result_path + "summary_student_ECG_EDA_KD.txt", "w")
