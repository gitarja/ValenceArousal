...
for s in subjects:
    ...
    for i in range(len(features_list)):
        ...
        features.append(np.concatenate([eda_features, ppg_features, resp_features, eeg_features, ecg_features]))
...
features_impt_ar = np.array([np.average(forest_ar.feature_importances_[0:eda_length]),
                             ...
                             np.average(forest_ar.feature_importances_[
...
plt.xticks(np.arange(len(features_impt_ar)), ["EDA", "PPG", "RESP", "EEG", "ECG"])
plt.show()
...
features_impt_val = np.array([np.average(forest_val.feature_importances_[0:eda_length]),
                             ...
                             np.average(forest_val.feature_importances_[
                                        eda_length + ppg_length:eda_length + ppg_length + resp_length]),
                             np.average(forest_val.feature_importances_[
                                        eda_length + ppg_length + resp_length:eda_length + ppg_length + resp_length + eeg_length]),
                             np.average(forest_val.feature_importances_[
