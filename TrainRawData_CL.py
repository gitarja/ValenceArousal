...
for fold in range(1, 2):
    ...
    result_path = CHECK_POINT_PATH + "ClassifyArVal_CL\\" + current_time + "\\"
    ...
    # data_fetch = DataFetchPreTrain_CL(training_data, validation_data, testing_data, ECG_RAW_N, eeg_raw=True, pretrain=False)
    data_fetch = DataFetchPreTrain_CL(validation_data, testing_data, testing_data, ECG_RAW_N, eeg_raw=True, pretrain=False)
    ...
    train_generator = tf.data.Dataset.from_generator(
        ...
        output_shapes=(
            tf.TensorShape([ECG_RAW_N]), tf.TensorShape([EEG_RAW_N, EEG_RAW_CH]), tf.TensorShape([2]),
            tf.TensorShape([2]),
            tf.TensorShape([4])))
    ...
    with strategy.scope():
        ...
        # Define model
        CL = ECGEEGEncoder()
        encoder_weight_path = CHECK_POINT_PATH + "ContrastiveLearning\\20210201-164643\\"
        input_ecg = tf.keras.layers.Input(shape=(ECG_RAW_N,))
        input_eeg = tf.keras.layers.Input(shape=(EEG_RAW_N, EEG_RAW_CH))
        # input_eeg = tf.keras.layers.Input(shape=(EEG_RAW_CH, EEG_RAW_N))
        CL.createModel(input_ecg, input_eeg)
        CL.ecg_encoder.load_weights(encoder_weight_path + "ECG_encoder_weights_1.hdf5")
        CL.eeg_encoder.load_weights(encoder_weight_path + "EEG_encoder_weights_1.hdf5")
        classification = ClassifyArVal_CL(num_output=NUM_OUTPUT, ecg_encoder=CL.ecg_encoder, eeg_encoder=CL.eeg_encoder,
        ...
        # accuracy
        train_acc_ar = tf.keras.metrics.CategoricalAccuracy()
        vald_acc_ar = tf.keras.metrics.CategoricalAccuracy()
        train_acc_val = tf.keras.metrics.CategoricalAccuracy()
        vald_acc_val = tf.keras.metrics.CategoricalAccuracy()
    ...
    with strategy.scope():
        ...
        for epoch in range(EPOCHS):
            ...
            # print("Arousal | Train | Loss: {}, Acc: {}, Precision: {}, Recall: {}".format(
            #     train_loss_ar.result().numpy(),
            #     train_acc_ar.result().numpy(),
            #     train_pre_ar.result().numpy(),
            #     train_rec_ar.result().numpy()))
            # print("Arousal | Validation | Loss: {}, Acc: {}, Precision: {}, Recall: {}".format(
            #     vald_loss_ar.result().numpy(),
            #     vald_acc_ar.result().numpy(),
            #     vald_pre_ar.result().numpy(),
            #     vald_rec_ar.result().numpy()))
            # print("Valence | Train | Loss: {}, Acc: {}, Precision: {}, Recall: {}".format(
            #     train_loss_val.result().numpy(),
            #     train_acc_val.result().numpy(),
            #     train_pre_val.result().numpy(),
            #     train_rec_val.result().numpy()))
            # print("Valence | Validation | Loss: {}, Acc: {}, Precision: {}, Recall: {}".format(
            #     vald_loss_val.result().numpy(),
            ...
            # Save model
            if prev_val_loss > vald_loss_ar.result().numpy():
                prev_val_loss = vald_loss_ar.result().numpy()
                wait_i = 0
                manager.save()
            else:
                wait_i += 1
