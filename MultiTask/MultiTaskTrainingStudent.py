...
from Conf.Settings import FEATURES_N, DATASET_PATH, CHECK_POINT_PATH, TENSORBOARD_PATH, ECG_RAW_N, \
    TRAINING_RESULTS_PATH, N_CLASS, ECG_N, N_SUBJECT
...
for fold in range(1, 6):
    ...
    train_generator = tf.data.Dataset.from_generator(
        ...
        output_shapes=(tf.TensorShape([FEATURES_N]), (tf.TensorShape([N_CLASS])), (), (), tf.TensorShape([ECG_N]), (), tf.TensorShape([N_SUBJECT]), ()))
    ...
    with strategy.scope():
        def train_step(inputs, shake_params, GLOBAL_BATCH_SIZE):
            ...
            with tf.GradientTape() as tape:
                t_em, t_r_ar, t_r_val, _, t_z, t_sub, t_gen = teacher_model(X_t, False)
                # t_em = tf.nn.sigmoid(t_em)
                # using latent
                # _, latent = base_model(X)
                z_em, z_r_ar, z_r_val, z, z_sub, z_gen = model(X, training=True)
