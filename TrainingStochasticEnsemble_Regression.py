...
from KnowledgeDistillation.Models.EnsembleFeaturesModel import EnsembleSeparateModel_Regression
...
for fold in range(1, 6):
    prev_val_loss = 1000
    wait_i = 0
    result_path = TRAINING_RESULTS_PATH + "Ensemble_Reg\\" + current_time + "\\fold_" + str(fold) + "\\"
    ...
    data_fetch = DataFetch(train_file=training_data, test_file=testing_data, validation_file=validation_data,
                           ECG_N=ECG_RAW_N, KD=False, multiple=True, soft=True, reg=True)
    ...
    with strategy.scope():
        model = EnsembleSeparateModel_Regression(num_output_val=num_output_val, num_output_ar=num_output_ar)
        ...
        # rmse
        train_ar_rmse = tf.keras.metrics.RootMeanSquaredError()
        train_val_rmse = tf.keras.metrics.RootMeanSquaredError()
        vald_ar_rmse = tf.keras.metrics.RootMeanSquaredError()
        vald_val_rmse = tf.keras.metrics.RootMeanSquaredError()
    ...
    with strategy.scope():
        def train_step(inputs, GLOBAL_BATCH_SIZE=0):
            ...
            with tf.GradientTape() as tape_ar:
                final_loss, prediction_ar, prediction_val = model.train(X, y_ar, y_val, GLOBAL_BATCH_SIZE,
                                                                        training=True)
            ...
            # accuracy
            train_ar_rmse(y_ar, prediction_ar)
            train_val_rmse(y_val, prediction_val)
        ...
        def test_step(inputs, GLOBAL_BATCH_SIZE=0):
            ...
            # print(prediction_ar)
            vald_ar_rmse(y_ar, prediction_ar)
            vald_val_rmse(y_val, prediction_val)
        ...
        def train_reset_states():
            ...
            train_ar_rmse.reset_states()
            train_val_rmse.reset_states()


        def vald_reset_states():
            vald_loss.reset_states()
            vald_ar_rmse.reset_states()
            vald_val_rmse.reset_states()
    ...
    with strategy.scope():
        ...
        for epoch in range(EPOCHS):
            ...
            with train_summary_writer.as_default():
                ...
                tf.summary.scalar('Arousal RMSE', train_ar_rmse.result(), step=epoch)
                tf.summary.scalar('Valence RMSE', train_val_rmse.result(), step=epoch)
            ...
            with test_summary_writer.as_default():
                ...
                tf.summary.scalar('Arousal RMSE', vald_ar_rmse.result(), step=epoch)
                tf.summary.scalar('Valence RMSE', vald_val_rmse.result(), step=epoch)
        ...
        template = (
            "Test_loss: {:.4f}, Test_RMSE_ar: {:.4f}, Test_RMSE_val: {:.4f}")
        print(template.format(vald_loss.result().numpy(), vald_ar_rmse.result().numpy(), vald_val_rmse.result().numpy()))
